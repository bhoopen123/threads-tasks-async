...
void MonitorQueue()
{
    while (isRunning || requestQueue.Count > 0)
    {
        if (requestQueue.Count > 0)
        {
            ...
            semaphore.Wait(); // instead of creating a new thread everytime, we use semaphore to ensure that only 3 thread can be created parallely. 
            // once, any of the previous thread is completed 
            // we will release the semaphore, so a new thread can be created
